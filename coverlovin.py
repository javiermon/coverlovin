...
def process_dir(thisDir):
    ...
    for file in files:
        ...
        if os.path.getsize(fileFullPath) > 128:
            ...
            try:
                fileInfo = mutagen.File(fileFullPath, easy=True)
            except:
                log.info("Skipping " + file + " due to unicode error")
                continue

            try:
                artist = fileInfo["artist"][0]
                album = fileInfo["album"][0]
            except:
				log.debug("Artist or album of " + file + " not found, continuing")
				continue

            log.info("artist: " + artist + ", album: " + album)
